...
library(e1071)
svmfit_linear <- svm(y~., data=dat1training, kernel ="linear", cost = 1,
              scale =FALSE)
...
svmfit_RBF <- svm(y~., data=dat1training, kernel ="radial", gamma = 10,
              cost = 1)
plot(svmfit_RBF , dat1training)
svmfit_poly <- svm(y~., data=dat1training, kernel="polynomial",degree = 2, gamma = 1,
                  cost = 1)
...
svmfit_poly <- svm(y~., data=dat2training, kernel="polynomial",degree = 2, gamma = 1,
                   cost =1)
...
svmfit_linear <- svm(Purchase~., data=datatraining, kernel ="linear", cost = 0.01,
                     scale =FALSE)
